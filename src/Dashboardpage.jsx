...
import { 
  DollarSign, ImagePlus, Brush, Eye, PlusCircle, 
  MessageCircle, Heart, CreditCard, BarChart2, 
  Clock, TrendingUp 
} from "lucide-react";
...
const DashboardPage = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 md:ml-64 p-6 sm:p-8">
        {/* Welcome Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Welcome back, Artist</h1>
          <p className="text-gray-600">Here's what's happening with your art business today</p>
        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-10 w-10">
                            <img className="h-10 w-10 rounded-full" src={sale.artworkImage} alt={sale.artworkName} />
                          </div>
                          <div className="ml-4">
                            <div className="text-sm font-medium text-gray-900">{sale.artworkName}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
              ...
              {recentActivities.map((activity) => (
                <div key={activity.id} className="flex items-start">
                  <div className={`flex-shrink-0 rounded-full p-2 ${
                    ...
                    {activity.type === 'sale' ? <DollarSign className="h-5 w-5" /> :
